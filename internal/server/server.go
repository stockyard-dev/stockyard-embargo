...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/embargos", s.list)
	s.mux.HandleFunc("POST /api/embargos", s.create)
	s.mux.HandleFunc("GET /api/embargos/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/embargos/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-embargo", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"embargos": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Embargo{} }
	wj(w, map[string]any{"embargos": items, "count": len(items)})
}
